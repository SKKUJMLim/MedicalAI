...
class CombinedUnet(nn.Module):
    ...
    def forward(self, x1, x2, x3):
            # 모델 각각에 이미지를 입력하여 latent vector 추출
            latent_vector1 = self.model1(x1)
            # print(latent_vector1.shape)(8,1,512,512)
            latent_vector1.squeeze()
            latent_vector2 = self.model2(x2)
            latent_vector2.squeeze()
            # 환자 정보 이미지
            latent_vector3 = self.model3(x3)
            #print(latent_vector3.shape)
            # 두 latent vector를 concat
            # print(latent_vector1.shape , latent_vector2.shape, latent_vector3.shape)
            combined = torch.cat((latent_vector1, latent_vector2, latent_vector3), dim=1)
